...
$$
\sigma=\rho<1\Rightarrow T(x_0)\leq\frac{V_0^{1-\rho}}{\beta(1-\rho)}<\infty,\ V_0:Q(V_0,x_0)=0,\ \beta=k-\delta>0,
$$
где $|d-\hat{d}|\leq\delta$ верхняя граница ошибки компенсации.
...
Так как $\ddot{y}=d+u/I$, то $d=\ddot{y}-u/I$.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=1]{yydu.png}
    \caption{Графики $y(t),\dot{y}(t),u(t)$}
    \label{fig:yydu}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=1]{fhf.png}
    \caption{Графики $d(t),\hat{d}(t)$}
    \label{fig:fhf}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=1]{efhf.png}
    \caption{График $e=d(t)-\hat{d}(t)$}
    \label{fig:efhf}
\end{figure}
\begin{figure}[H]
    \centering
...
Система достигла установившегося состояния за $t=1.5$ секунды.


Покажем разницу асимптотической и финитной динамик при $e(t)\equiv s(t)$.
...
s(t)=s(0)e^{-kt}
$$
Скользящая переменная убывает экспоненциально, но $s(t)=0$ только при $t\to\infty$.


Финитная динамика:
$$
\dot{s}=-k|s|^{\rho}\operatorname{sign}(s),\ 0<\rho<1
$$
Решение:
$$
s(t)=0\text{ при }t=\frac{|s(0)|^{1-\rho}}{k(1-\rho)}<\infty
$$
Скользящая переменная становится нулевой за конечное время $t$.


\section{Вывод}
В ходе выполнения лабораторной работы
была выбрана нелинейная управляемая система с одним входом
и одним выходом и представлена в виде цепи интеграторов с
согласованной обобщенной неизвестной динамикой. Далее
был синтезирован финитный однородный регулятор
с дополнительной компенсацией обобщенной неизвестной динамики.
Результаты моделирования подтверждают корректность расчетов.
\end{document}