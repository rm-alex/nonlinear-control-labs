...
$$
V=0.5s^2,\ \dot{V}\leq-\beta_0|s|=-\beta_0\sqrt{2V}<0\,\forall s\neq0
$$


Поверхность $s=0$ достигается за конечное время.


При $|s|\leq\varepsilon:\operatorname{sat}(s/\varepsilon)=s/\varepsilon$.
Воспользуемся ранее найденной $\dot{s}$ с оцененной $|\delta(x)|\leq\rho(x)$:
$$
\dot{s}=3u_\text{sw}+\rho(x)
$$


Заменим в разрывной части управления $\operatorname{sign}(s)$
на $s/\varepsilon$:
$$
u_\text{sw}=-\beta(x)\frac{s}{\varepsilon}
$$


Тогда:
$$
\dot{s}=-3\beta(x)\frac{s}{\varepsilon}+\rho(x)
$$
