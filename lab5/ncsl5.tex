...
$$
|\delta(x)|\leq a|x_1\sin{x_1}|+|x_1x_2|\leq ax_1^2+|x_1x_2|
$$


Тогда:
$$
\dot{s}=-3\beta(x)\operatorname{sign}(s)+ax_1^2+|x_1x_2|
$$


Оценим производную функции Ляпунова $V=0.5s^2$:
$$
\dot{V}=s\dot{s}=s\left( -3\beta(x)\operatorname{sign}(s)+\delta(x) \right)=-3\beta(x)|s|+\delta(x)s,
$$
$$
\dot{V}=-3\beta(x)|s|+\delta(x)s\leq-3\beta(x)|s|+|\delta(x)||s|=-\left( 3\beta(x)-|\delta(x)| \right)|s|
$$


Если выбрать $3\beta(x)\geq|\delta(x)|+\beta_0,\ \beta_0>0$:
$$
...
Моделирование системы при $a=2,\beta_0=0.8,\varepsilon=0.01,x_0=\left[ 1\ 0 \right]^T,a_1=0.5,a_2=1$:
\begin{figure}[H]
    \centering
