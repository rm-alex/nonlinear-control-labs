...
Область, где производная отрицательна:
$$
r^2=x_1^2+x_2^2<1
$$
...
Тогда:
$$
\forall\left( x_1,x_2 \right)\neq\left( 0,0 \right),x_1^2+x_2^2<1:\dot{V}<0
$$
...
Область, в которой производная отрицательна:
$$
r^2=x_1^2+x_2^2<1
$$
...
Функция Ляпунова:
$$
V(x)=x^TPx,\ x=\begin{bmatrix}
    x_1&x_2
\end{bmatrix}^T,
$$
где $P$ -- симметрическая матрица.


Найдем $P$:
$$
A^TP+PA=-I,\ A=\begin{bmatrix}
    -3&-1\\2&0
...
$P\succ0\,\left[ \lambda_{1,2}=\left(\pm\sqrt{5}+3\right)/4 \right]$
-- $V(x)$ положительно определена и эквивалентна $||x||^2$.
...
$$
\dot{V}=x^T\left( A^TP+PA \right)x+2x^TPg(x)=-||x||^2+2x^TP\begin{bmatrix}
    0&-x_2^3
\end{bmatrix}^T
$$


Оценим остаточный член по модулю.


Обозначим операторную норму матрицы $P$
через $||P||$ (наибольшее собственное значение $P$):
$$
||P||=\frac{\sqrt{5}+3}{4}
$$
...
Тогда:
$$
\bigg|2x^TP\begin{bmatrix}
    0&-x_2^3
\end{bmatrix}^T\bigg|\leq 2||x||\,||P||\,\bigg|\bigg|\begin{bmatrix}
    0&-x_2^3
\end{bmatrix}\bigg|\bigg|\leq 2||P||\,||x||\,|x_2|^3\leq 2||P||\,||x||^4
$$


Производная функции Ляпунова:
$$
\dot{V}(x)\leq -||x||^2+2||P||\,||x||^4=-||x||^2\left( 1-2||P||\,||x||^2 \right)
$$


Положим радиус окрестности нуля $r>0$ такой, что:
$$
2||P||r^2\leq0.5\Rightarrow r\leq0.43,\ r^2=x_1^2+x_2^2\leq0.43^2
$$


Тогда, для всех $x\neq0$ с $||x||\leq r$:
$$
\dot{V}(x)\leq-0.5||x||^2<0
$$


Таким образом, начало координат локально асимптотически устойчиво.
...
\subsection{Выполнение}
% тут нужно через ляпунова, там несколько строчек получится
...
$$
\bigg|\frac{h(x)}{x^p}\bigg|\leq k|x|,\ k>0
$$
...
Учтем для $|x|$:
$$
\frac{d|x|}{dx}=\dot{x}\operatorname{sign}\left( x \right),\ x^p\operatorname{sign}\left( x \right)=|x|^p,\ p=2n+1,\ n\in\mathbb{N}_0,
$$
$$
\dot{x}\operatorname{sign}\left( x \right)=\left( \alpha x^p +h(x) \right)\operatorname{sign}\left( x \right)=x^p\operatorname{sign}\left( x \right)\left( \alpha+\frac{h(x)}{x^p} \right)
$$


Следовательно, для $|x|<\varepsilon$:
$$
\dot{x}\operatorname{sign}\left( x \right)=|x|^p\left( \alpha+\frac{h(x)}{x^p} \right)\leq\frac{\alpha}{2}|x|^p<0,
$$
то есть $|x|$ монотонно убывает вдоль траекторий,
пока решение остаётся в окрестности нуля.


Таким образом, $|x(t)|\xrightarrow[t\to\infty]{}0$ при малых $x$, $p=2n+1, n\in\mathbb{N}_0$ и $\alpha<0$ -- начало координат локально асимптотически устойчиво.


Также это задание можно решить через функцию Ляпунова $V=0.5x^2$.
...
В качестве $d$ учитываем шумы измерений или аддитивные возмущения.


В доказательстве глобальной устойчивости полагаем $d=0$.


Функция Ляпунова:
$$
V=\frac{k_1}{2}x_1^2+\frac{1}{2}x_2^2,\ V\left( x_1,x_2 \right)>0\, \forall \left( x_1,x_2 \right)\neq\left( 0,0 \right),k_1>0,\ V\left( 0,0 \right)=0
$$


Ее производная:
$$
\dot{V}=k_1x_1\dot{x_1}+x_2\dot{x}_2=k_1x_1\left( x_2-0.5x_1^3 \right)+x_2\left( -k_1x_1-k_2x_2 \right)
=-0.5k_1x_1^4-k_2x_2^2
$$


Член $k_1x_1x_2$ сократился.


При всех $k_1,k_2>0$:
$$
\dot{V}<0,\ \dot{V}=0:\left( x_1,x_2 \right)=\left( 0,0 \right)
$$
...
При наличии шумов измерений вектор состояния объекта:
$$
\tilde{x}=x+n(t),
$$
где $n(t)$ -- векторный шум.


Управление:
$$
u=K\tilde{x}=Kx+Kn(t)=Kx+w(t)
$$


Производная функции Ляпунова:
$$
\dot{V}=k_1x_1\left( x_2-0.5x_1^3 \right)+x_2\left( -k_1x_1-k_2x_2+w(t) \right)
=-0.5k_1x_1^4-k_2x_2^2+x_2w(t)
$$


Неравенство Коши-Шварца:
$$
ab\leq \frac{\varepsilon}{2}a^2+\frac{1}{2\varepsilon}b^2,\ a,b\in\mathbb{R},\varepsilon>0
$$


Положим $a=|x_2|,b=|w(t)|,\varepsilon=k_2>0$:
$$
x_2w(t)\leq |x_2||w(t)|\leq \frac{k_2}{2}x_2^2+\frac{1}{2k_2}|w(t)|^2
$$


Тогда:
$$
\dot{V}\leq -0.5k_1x_1^4-k_2x_2^2+\frac{k_2}{2}x_2^2+\frac{1}{2k_2}|w(t)|^2=
-0.5k_1x_1^4-0.5k_2x_2^2+\frac{1}{2k_2}|w(t)|^2
$$


Получили:
$$
\dot{V}\leq -\alpha\left( |x| \right)+\gamma\left( |w(t)|^2 \right),
$$
где $\alpha\left( |x| \right)$ -- положительно определенная функция,
$\gamma\left( |w(t)|^2 \right)$ -- положительная функция входа.


Пусть:
$$
||w||_{\infty}:=\sup{}_t{|w(t)|}\leq W
$$


Тогда при $\dot{V}\geq0$:
$$
0.5k_1x_1^4+0.5k_2x_2^2\leq\frac{1}{2k_2}W^2
$$


Отдельно для компонент:
$$
0.5k_1x_1^4\leq\frac{1}{2k_2}W^2\Rightarrow |x_1|\leq \left( k_1k_2 \right)^{-1/4}W^{1/2},
$$
$$
0.5k_2x_2^2\leq\frac{1}{2k_2}W^2\Rightarrow |x_2|\leq \frac{W}{k_2}
$$


То есть $x_1$ ограничено пропорционально $\sqrt{W}$, $x_2$ линейно по $W$.


Таким образом, при наличии шумов измерений система
устойчива по входу-к-состоянию (ISS) -- при любом
ограниченном входе $w(t)$ состояния $x_1,x_2$
тоже остаются ограниченными.


При наличии аддитивных возмущений уравнение динамики $x_2$:
$$
\dot{x}_2=-k_1x_1-k_2x_2+d(t)
$$


Этот случай аналогичен случаю с измерительным шумом $w(t)$.


Производная функции Ляпунова:
$$
\dot{V}=-0.5k_1x_1^4-k_2x_2^2+x_2d(t)
$$


Аналогичные преобразования:
$$
\dot{V}\leq -0.5k_1x_1^4-0.5k_2x_2^2+\frac{1}{2k_2}|d(t)|^2
$$


Ограничения для компонент будут аналогичны $W\sim|d|_{\infty}$.


Таким образом, при наличии аддитивных возмущений система устойчива
по входу-к-состоянию. Ограниченный возмущающий сигнал дает ограниченное состояние.
...
\subsection{Выполнение}
Из условия $y\sigma\left( y \right)\geq0$ следует, что $\sigma\left( y \right)$ имеет тот же знак, что и $y$.
...
Ее производная:
$$
\dot{V}=x_1\left( -2x_1+x_2 \right)+x_2\left( -x_1-\sigma\left( x_1 \right)-x_2+d \right)=
-2x_1^2-x_2^2-x_2\sigma\left( x_1 \right)+x_2d
$$


При $d=0$ член $x_2\sigma\left( x_1 \right)$ может иметь произвольный знак,
поэтому производная функции Ляпунова не всегда строго отрицательна.


Неравенство Коши-Буняковского:
$$
x_2d\leq\frac{1}{2}x_2^2+\frac{1}{2}d^2
$$


Тогда:
$$
\dot{V}\leq -2x_1^2-x_2^2-x_2\sigma\left( x_1 \right)+\frac{1}{2}x_2^2+\frac{1}{2}d^2=
-2x_1^2-\frac{1}{2}x_2^2-x_2\sigma\left( x_1 \right)+\frac{1}{2}d^2
$$


Так как $\sigma(0)=0$, то для $x_2=0$ можно для малых $x_1$ записать:
$$
|\sigma\left( x_1 \right)|=|\sigma\left( x_1 \right)-\sigma\left( 0 \right)|\leq L|x_1-0|=L|x_1|,
$$
где $L$ -- константа Липшица.


В окрестности нуля функция растет не быстрее, чем $L|x_1|$.


Оценим член $-x_2\sigma\left( x_1 \right)$:
$$
|-x_2\sigma\left( x_1 \right)|\leq |x_2||\sigma\left( x_1 \right)|\leq L|x_1||x_2|\leq \frac{L}{2}\left( x_1^2+x_2^2 \right)
$$


Тогда производная функции Ляпунова:
$$
\dot{V}\leq -2x_1^2-\frac{1}{2}x_2^2+\frac{L}{2}x_1^2+\frac{L}{2}x_2^2+\frac{1}{2}d^2=
-\left( 2-\frac{L}{2} \right)x_1^2-\left( \frac{1}{2}-\frac{L}{2} \right)x_2^2+\frac{1}{2}d^2
$$


Скобки должны быть положительны:
$$
\begin{cases}
    2-\frac{L}{2}>0,\\
    1-L>0
\end{cases}\Rightarrow L<1
$$


Положительно определенная функция:
$$
\alpha=\min{\left( \alpha_1,\alpha_2 \right)}>0,\ \alpha_1=2-L/2, \alpha_2=1/2-L/2
$$


Обозначим:
$$
||x||^2= x_1^2+x_2^2,\ \gamma\left( s \right)=\frac{1}{2}s,s=||d||^2
$$


Тогда производная функции Ляпунова:
$$
\dot{V}\leq -\alpha||x||^2+\frac{1}{2}d^2=-\alpha||x||^2+\gamma\left( ||d||^2 \right)
$$


Ограничение величины состояния при $ \dot{V}\geq0$:
$$
||x||^2\leq \frac{1}{2\alpha}d^2\Rightarrow ||x||\leq \frac{1}{\sqrt{2\alpha}}|d|
$$


Величина состояния ограничена линейно по $|d|$.


Таким образом, система устойчива по входу-к-состоянию (ISS) при $L<1$.
С возмущением $d\neq0$ состояние остается ограниченным,
величина пропорциональна размеру $d$.


При $d=0$ система асимптотически устойчива.


\section{Вывод}
В ходе данной лабораторной работы были
исследованы различные нелинейные системы.
Устойчивость определялась по
производной функции Ляпунова.
Был синтезирован регулятор с условием
на параметр для степени экспоненциальной устойчивости.
Была исследована устойчивость
системы с регулятором при наличии шумов измерений и
аддитивных возмущений. Была исследована устойчивость
по входу-к-состоянию системы по отношению к возмущению.
Каждый результат был обоснован.
\end{document}